...
- 결정: `guarded-bugfix` 스킬은 메인 대화에서 직접 진단·수정·검증·범위 한정 커밋을 수행한다. 후보 수정 파일 또는 인덱스가 다른 작업으로 변경돼 있으면 아무 것도 수정하지 않고 중단하며, 해당 작업이 커밋된 뒤 사용자가 `진행해`라고 하면 상태를 다시 확인한 뒤에만 재개한다. 단, `SESSION-HANDOFF.md`는 읽기 전용 공유 로그로 취급해 수정·스테이징·충돌 판정 대상에서 제외한다.
- 영향: `C:\Users\byh02\.codex\skills\guarded-bugfix\SKILL.md`, 버그 수정의 충돌·재개·커밋 절차
