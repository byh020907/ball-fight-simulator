...
## [L1] 2026-07-12 — 업적별 보상 핸들러 등록
- 맥락: 보상 종류별 중앙 분기는 새 업적의 개별 동작을 추가할 때마다 공통 분기 코드를 수정하게 만든다.
- 결정: 업적 ID마다 전용 보상 핸들러를 등록하고, 마스터 보상 핸들러는 업적 정의를 받아 해당 등록 핸들러에만 위임한다. 실제 파편, 상자, 장비 지급의 공통 원자 동작만 재사용한다.
