...
- 확정값: 벽 법선 기준 입사각 25° 이하에서 도달 0.25초 전에 생성한다. 길이 120px 경사면은 벽 안쪽 55px에 두고 법선 기준 ±25~45°로 기울이며, 실제 충돌 직후 또는 0.7초 뒤 제거한다. 같은 공은 1.2초 뒤에만 다시 요청할 수 있고 경기장에는 동시에 하나만 유지하며, 방향 난수는 경기 시드로 재현한다.
- 영향: 토너먼트 전용 전투 시뮬레이션 옵션과 임시 지형 수명·렌더링, 다각형 지형 충돌, 회귀/수치 시뮬레이션, 전투 규칙·도움말·패치노트·자산 캐시 버전.
- 구현: `src/tournament/angledBounceRamps.js`가 다음 벽 예측·생성·수명·쿨다운을 소유하고, 실제 토너먼트 `BattleSimulation`만 이를 활성화한다. 공통 다각형 충돌 콜백으로 제거하며, 사냥터·프리뷰·일반 매치는 기본 비활성이다.
