...
- 확정값: 벽 법선 기준 입사각 25° 이하에서 도달 0.25초 전에 생성한다. 길이 120px은 벽에 붙는 변이고, 직각삼각형은 경기장 안쪽 55px까지 뻗는다. 실제 충돌 직후 또는 0.7초 뒤 제거하며, 같은 공은 1.2초 뒤에만 다시 요청할 수 있고 경기장에는 동시에 하나만 유지한다. 삼각형의 위·아래 방향 난수는 경기 시드로 재현한다.
- 영향: 토너먼트 전용 전투 시뮬레이션 옵션과 임시 지형 수명·렌더링, 다각형 지형 충돌, 회귀/수치 시뮬레이션, 전투 규칙·도움말·패치노트·자산 캐시 버전.
- 구현: `src/tournament/angledBounceRamps.js`가 다음 벽 예측·벽 부착 직각삼각형 생성·수명·쿨다운을 소유하고, 실제 토너먼트 `BattleSimulation`만 이를 활성화한다. 공통 다각형 충돌 콜백으로 제거하며, 생성과 제거는 기존 `GravityParticle` 파티클만 방출한다. 사냥터·프리뷰·일반 매치는 기본 비활성이다.
- 후속 결정: 짧은 직사각형 경사면 대신 벽선 한 변에 붙는 직각삼각형 장애물을 사용한다. 기존 길이 `120px`은 벽에 붙는 변, `55px`은 경기장 안쪽 깊이로 해석하며, 생성과 충돌·만료 제거 시 실제 위치에 파티클 기반 이펙트를 낸다.
