...
- 결정: 장비 탭 우측 상단의 `상점` 버튼으로 전용 팝업을 연다. 상점은 파편 150 상자를 6시간 주기당 2개 판매하며, 사냥터보다 낮은 품질 풀(common 70%, uncommon 25%, rare 5%)만 제안한다. 상자 제안은 파편 30부터 매회 30씩 올라 최대 300(기본 비용의 10배)까지 리롤하고, 리롤 비용은 1시간 30분마다 초기화한다. 리롤한 상자는 기존 `<chest-icon>`을 재사용하며, `x-if` 내부처럼 동적으로 생성되는 상자에는 `x-component="'chest-icon'"`을 명시해 마운트를 보장한다. 등급이 같아도 상자 자체가 튀어 오르고 뚜껑이 열렸다 닫히는 교체 연출을 재생한다. 상점 팝업은 구매·리롤 후에도 유지한다.
