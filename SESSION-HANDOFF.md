...
- 구현 진행: UI 액션 브리지 이관은 `837f78e`, 결과 전용 `game-overlay`와 사냥 흐름 전용 `hunting-overlay` 분리는 `ae00a7e`로 완료했다. 정적 UI 계약은 `tests/uiContracts.mjs`로 분리해 시뮬레이션 회귀 러너와 독립 실행한다. 다음 단위는 컬렉션 허브의 기능별 템플릿 책임 분리다.
