...
## [L1] 2026-07-12 — Hero Orb 자석은 발사 뒤 1초 후 Hero 반지름 2배와 Orb 반지름 범위에서 시작
- 맥락: Hero Lv.3 귀환 자석이 오브 생성 직후부터 발사 방향과 반대인 Hero 쪽 속도 보정을 적용해 초기 투사 속도를 줄였고, 영향 반경도 Hero와 Orb의 합산 반지름에 3배를 곱해 의도보다 넓었음.
- 결정: Hero가 발사하거나 방출한 오브는 생성 뒤 첫 1초 동안 자석 대상에서 제외한다. 유예 뒤 자석 영향 반경은 `Hero Ball 반지름 × 2 + Orb 반지름`으로 계산해, Hero 쪽 기준 반경과 Orb 자체 크기를 함께 반영한다. 실제 획득은 계속 본체 충돌에서만 처리한다.
