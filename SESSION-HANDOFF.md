...
## [L2] 2026-07-12 — 접지 접미사 보류
- 배경: 마찰 보정은 실제 물리값이지만 현재 평면 볼 전투에서는 접선 충돌에서만 약하게 드러나며, 충돌 후 전투 템포를 떨어뜨릴 가능성이 있음.
- 결정: `접지`는 MVP 특수 접미사 풀에서 제외한다. 얼음·모래·진흙 등 지형 재질이 전략에 직접 관여할 때 지형 마찰 대응 옵션으로 재검토한다.
- 영향: 현재 물리 접미사 MVP는 중량·반향·소용돌이로 한정.
