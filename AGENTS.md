...
### 시작 루틴
**새 대화 시작 시 가장 먼저** `SESSION-HANDOFF.md`(워크스페이스 루트)를 읽어 현재 계약, 열린 리스크, 활성 결정을 확인합니다. 과거 결정의 배경이 필요할 때만 `docs/decision-history.md`를 검색합니다.

### 결정 기록 (Decision Log)
`SESSION-HANDOFF.md`는 활성 인계를, `docs/decision-history.md`는 현행 문서에 흡수된 전체 결정 이력을 관리합니다.
**결정이 발생할 때마다 `SESSION-HANDOFF.md`에 실시간으로 기록**하며, 단순 작업 목록이 아니라 아직 활성 문맥이 필요한 결정만 유지합니다.
...
- 문서 현행화 시 현재 코드와 문서가 결정의 내용·이유를 완전히 보존하는지 확인하고, 완전히 흡수된 결정만 `docs/decision-history.md`로 이동
- 일부만 반영됐거나 열린 리스크가 남은 결정은 `SESSION-HANDOFF.md`에 유지
- 폐기·대체된 결정도 삭제하지 않고 전체 이력에 보존하며 대체 결정을 함께 기록

#### 선제 정리 요청
새 대화 시작 시와 L1/L2 결정을 추가한 직후 `SESSION-HANDOFF.md`의 활성 크기를 확인합니다. 아래 기준 중 하나라도 충족하면 같은 세션에서 한 번만 실제 줄 수·활성 결정 수·UTF-8 파일 크기를 알려 주고 `결정 기록 정리`를 선제 제안합니다.

- 200줄 이상
- `## [L1]`·`## [L2]` 활성 결정 합계 20개 이상
- UTF-8 파일 크기 32 KiB 이상

임계값 도달은 자동 이동 권한이 아닙니다. 사용자가 승인하기 전에는 결정을 `docs/decision-history.md`로 옮기지 않으며, 정리 제안은 현재 요청을 막지 않는 진행 알림으로 전달합니다. 승인되면 각 활성 결정의 내용·이유·열린 리스크를 점검하고, 현행 문서에 완전히 흡수된 결정만 전체 이력으로 이동합니다.
...
6. **문서 현행화 트리거**: 사용자가 `문서 현행화`, `문서 정합`, 또는 같은 의미의 요청을 하면 `docs/development-rules.md`의 문서 현행화 절차를 즉시 적용합니다. 부분 수정으로 정합 가능한 문서는 `docs/`에 유지하고, 전면 재작성해야 현행 계약이 되는 문서만 `.legacy/docs/`로 이동합니다.
...
`src/helpContent.js` — 게임 내 도움말 / `src/patchNotes.js` — 패치노트
