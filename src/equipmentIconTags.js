...
registerTag("intermediate_attack_crit", "중간 · 공격·치명타 · 정밀 보강검", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("intermediate_attack_haste", "중간 · 공격·스킬 가속 · 마력 동력검", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("intermediate_attack_speed", "중간 · 공격·속도 · 경량 날개검", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("intermediate_attack_health", "중간 · 공격·HP · 생명 수정 절단구", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("intermediate_health_defense", "중간 · HP·방어 · 수정 가죽흉갑", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("intermediate_health_haste", "중간 · HP·스킬 가속 · 열맥동 허리장치", (ctx, cx, cy) => {
    const S = 0.06;
    drawOpenPath(
        ctx,
        [
...
registerTag("intermediate_defense_wall", "중간 · 방어·벽 반사 속도 · 스프링 완충판", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("intermediate_defense_mass", "중간 · 방어·질량 · 중량 사슬조끼", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("intermediate_speed_wall", "중간 · 속도·벽 반사 속도 · 스프링 도약화", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("intermediate_speed_angular", "중간 · 속도·회전 충격 · 날개 자이로 안정기", (ctx, cx, cy) => {
    const S = 0.06;
    ctx.beginPath();
...
registerTag("intermediate_haste_angular", "중간 · 스킬 가속·회전 충격 · 점화 자이로 코어", (ctx, cx, cy) => {
    const S = 0.06;
    [
        [S * 5.8, S * 2.5, 0.45],
        [S * 4.2, S * 1.7, -0.45]
...
registerTag("intermediate_crit_mass", "중간 · 치명타·질량 · 쌍날 충격두", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("completed_ability_crit", "완성 · 스킬·치명타 폭발 · 별을 꿰는 서약", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("completed_pursuit_flurry", "완성 · 추격·연타 · 쌍익의 질풍", (ctx, cx, cy) => {
    const S = 0.06;
    [-1, 1].forEach((side) => {
        drawClosedPath(
            ctx,
...
registerTag("completed_mass_execution", "완성 · 질량·치명타 처형 · 종언의 추락", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("completed_vital_heat", "완성 · HP·열기 지속 공격 · 홍련의 맥동", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("completed_defense_conversion", "완성 · 방어·공격 전환 · 철혈의 송곳니", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("completed_mass_shockwave", "완성 · 질량·광역 충격파 · 낙성의 파문", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("completed_wall_ricochet", "완성 · 벽 반사·도탄 공격 · 되튀는 초승달", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("completed_wall_heat", "완성 · 벽 반사·열기 파동 · 화염심장 성채", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
...
registerTag("completed_speed_angular", "완성 · 속도·회전 충격 · 천공의 나선", (ctx, cx, cy) => {
    const S = 0.06;
    ctx.beginPath();
...
registerTag("completed_ability_echo", "완성 · 스킬·충돌 메아리 · 쌍성의 메아리", (ctx, cx, cy) => {
    const S = 0.06;
...
registerTag("completed_vortex_charge", "완성 · 이동·광역 회전 · 폭풍의 윤환", (ctx, cx, cy) => {
    const S = 0.06;
    ctx.beginPath();
...
registerTag("completed_vital_overwhelm", "완성 · HP·충돌 압도 · 적룡의 심갑", (ctx, cx, cy) => {
    const S = 0.06;
    drawClosedPath(
        ctx,
        [
