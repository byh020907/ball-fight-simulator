...
export class MobAppearance {
    ...
    static getTemplate(name) {
        return MobAppearance.TEMPLATES[name] ?? MobAppearance.TEMPLATES.default;
    }

    static generate(rng = Math.random) {
        const sides =
            Math.floor(rng() * (MobAppearance.SHAPES.MAX_SIDES - MobAppearance.SHAPES.MIN_SIDES + 1)) +
            MobAppearance.SHAPES.MIN_SIDES;
        const faceNames = Object.keys(MobAppearance.TEMPLATES);
        const face = faceNames[Math.floor(rng() * faceNames.length)];
