...
방벽 볼은 4초마다 방벽을 켠 뒤 1.5초 동안 아군과 실제로 충돌했을 때만 위치를 교환한다. 충돌한 아군이 플레이어에 더 가까울 때만 방벽 볼이 그 자리로 나서고, 뒤쪽 아군과의 충돌은 그대로 밀려난다. 그래서 멀리 있는 아군을 순간이동시키지 않고, 아군이 진로를 막는 순간에만 방벽 볼이 전열을 맡는다. 방벽 시간에는 기존처럼 방어력이 상승하고, 교체 순간은 파란 연결선과 양쪽 파동으로 표시한다.
