...
| ID | 주력 전환 층 | 행동 | 대표 연출 |
|---|---:|---|---|
| `pursuer` | 1 | 플레이어를 계속 추적 | 화난 표정의 갈색 공 |
| `charger` | 1 | 주기적으로 목표 방향으로 돌진 | 대시 표정과 붉은 돌진 |
| `shooter` | 10 | 목표를 향해 화살 발사 | 외눈 표정과 투사체 |
| `electric` | 20 | 사거리 안 대상에게 지속 전기 피해 | 하늘색 전기 연결선 |
| `healer` | 30 | 아군 한 명에게 자신의 HP를 써서 회복 | 초록 회복 빔과 행복한 표정 |
| `chain` | 40 | 대상과 사슬을 연결해 압박 | 붉은 연결선 |
| `shockwave` | 50 | 주변을 밀어내는 충격파 | 넓어지는 원형 파동 |
| `barrier` | 60 | 잠시 방어력을 높이는 방벽 | 파란 보호 고리 |
| `siphon` | 70 | 대상 HP를 흡수 | 보라색 흡수 연결선 |
| `shard` | 80 | 부채꼴 파편 3발 발사 | 노란 파편 투사체 |
| `boomerang` | 91 | 적중 뒤에도 돌아오는 투사체 | 주황 귀환 공 |
| `splitter` | 92 | 여러 방향으로 분열 파편 발사 | 보라색 분열 탄막 |
| `jumper` | 93 | 목표를 향해 도약 | 커졌다 작아지는 몸체 |
| `laser` | 94 | 충전 시 고정한 각도로 직선 레이저 발사 | 붉은 충전선과 굵은 레이저 |

코드의 단일 데이터 원본은 `src/hunting/huntingMonsters.js`의 `CAVE_MONSTERS`다. 주력 전환 층·가중치·외형·기본 스탯을 수정할 때는 이 표와 함께 갱신한다.
