...
- 소형 힐팩은 회수 순간의 잃은 체력 25%를 회복한다. 일반 파편은 1층부터 25층마다 5씩 증가하는 5~20 기준값을 사용한다. 파편 꾸러미는 이 층수 기준값에 rare `1~2배`, unique `1~2.5배`, epic `1~3.5배`를 무작위로 적용하고 중앙값이 양끝보다 자주 나오게 한다. 모든 파편 결과는 5 단위로 정규화한다. 이로써 깊은 층의 보상 성장이 몬스터 등급보다 경제에 더 크게 반영된다.
