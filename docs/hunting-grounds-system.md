...
| `laser` | 레이저 볼 | 94 | 충전 시 고정한 각도로 직선 레이저 발사 | 붉은 충전선과 굵은 레이저 |

코드의 단일 데이터 원본은 `src/hunting/huntingMonsters.js`의 `CAVE_MONSTERS`다. 전투 표시명·주력 전환 층·가중치·외형·기본 스탯을 수정할 때는 이 표와 함께 갱신한다.

레이저 몬스터는 충전 시작 시 목표 각도를 고정한 뒤 벽 반사 직선 레이저를 유지하며, 힐러는 반경 안 아군 한 명에게만 초록 회복 빔을 연결해 자기 HP 100/s를 대상 HP 300/s로 전환한다. 힐러는 별도로 최대 HP의 5%/s를 자연 회복한다.
