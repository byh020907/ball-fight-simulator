...
사냥터 몹(다각형 외형)은 `RotationalBody` 믹스인을 통해 무작위 회전각과 각속도를 가지며, `_drawPolygonBody` 렌더링에 `this.angle`이 적용됩니다. 몹-vs-플레이어 충돌도 원형 radius가 아닌 polygon shape 기반으로 처리됩니다: `resolveFighterShapeCollision`이 SAT 알고리즘으로 polygon-polygon, circle-polygon 충돌을 감지하고, SAT normal 방향으로 충돌 반응(impulse)을 주며 분리합니다. 분리 시 bounding circle overlap을 하한으로 사용하여 완전한 분리를 보장합니다.
