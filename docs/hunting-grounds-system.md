...
| `electric` | 전기 마법사 | 20 | 사거리 안 대상에게 지속 전기 피해 | 대상까지 계속 꺾이며 움직이는 하늘색 번개 |
...
전기 마법사의 번개는 `src/effects/electricArc.js`가 시작점과 끝점 사이를 거리 기반 절점으로 나누고, 각 절점을 시간에 따라 수직으로 흔들어 그린다. 공통 렌더러는 외곽 광선·색 본선·흰 심선을 겹치므로, 다음 연결형 공격도 같은 아크 표현을 재사용할 수 있다. 전기·힐·사슬·흡수와 방벽 교대 같은 연결형 효과는 대상이 처치·파괴·삼킴 상태가 되면 즉시 상태를 해제하며, 같은 프레임 렌더링에서도 선을 남기지 않는다.
