...
stage는 단순 배경 외에 실제 충돌 지형을 가질 수 있습니다. 1차 구현에서는 동굴(cave) stage에 원형(3~4개) 및 폴리곤(홀수 층 1개) 암벽 장애물이 생성되며, fighter와 충돌 시 arena 벽 반사와 동일한 방식으로 밀려납니다. 충돌은 원형 entity vs circle/polygon terrain을 모두 지원합니다 (SAT 기반 polygon 충돌). 지형은 `stageId`와 `floor`에 기반해 결정론적으로 생성되므로 같은 조건에서는 항상 같은 배치가 나옵니다. 투사체 충돌, 몹 AI의 장애물 우회, concave polygon, 동적 회전 장애물은 후속 과제입니다.
