...
일반 사냥터 몬스터가 처치되면 전장에 `소형 힐팩`, `일반 파편`, `파편 꾸러미`, `일반 상자`, `고급 상자` 중 하나를 떨어뜨릴 수 있다. 아이템은 처치 즉시 보상으로 합산하지 않으며, 플레이어가 전장에서 실제로 닿아 회수한 뒤에만 이번 전투의 임시 보상에 기록한다. 패배하면 이 임시 보상은 폐기되고, 승리하면 **회수한 전리품만** 사냥터 런의 pending loot으로 병합한다. 전투 승리 뒤 별도로 계산해 지급하는 파편 보상은 없다.

- 일반 몬스터는 하나의 드롭 판정만 가진다. 기본 확률은 15%이고, 플레이어의 잃은 체력 비율에 따라 선형으로 증가해 체력이 0에 가까울수록 최대 30%(기본의 2배)가 된다. 고등급 몬스터에 추가 독립 보상 판정을 더하지 않는다.
- 드롭 판정을 통과한 뒤에는 몬스터 태그의 `rarity:common|rare|unique|epic`에 맞는 하나의 보상 테이블을 고른다. common은 일반 테이블만 사용하고, 파편 꾸러미와 고급 상자는 rare부터 나온다.
...
- 일반 테이블은 잃은 체력 비율 `m`에 따라 소형 힐팩 `20 + 20m`, 일반 상자 `10`, 일반 파편 `70 - 20m`의 비율을 쓴다. 등급별 특별 보상이 먼저 차지한 뒤에는 이 비율을 남은 확률에 맞게 비례 축소한다. 만피여도 힐팩 확률을 파편으로 치환하지 않으며, 회복할 체력이 없는 힐팩은 회수되지 않는다.
