...
일반 사냥터 몬스터가 처치되면 전장에 층수에 따라 변하는 랜덤 수량의 `일반 파편`을 반드시 떨어뜨리고, `소형 힐팩`, `파편 꾸러미`, `일반 상자`, `고급 상자` 중 하나를 추가로 떨어뜨릴 수 있다. 아이템은 처치 즉시 보상으로 합산하지 않으며, 플레이어가 전장에서 실제로 닿아 회수한 뒤에만 이번 전투의 임시 보상에 기록한다. 패배하면 이 임시 보상은 폐기되고, 승리하면 **회수한 전리품만** 사냥터 런의 pending loot으로 병합한다. 전투 승리 뒤 별도로 계산해 지급하는 파편 보상은 없다.

- 일반 몬스터는 층수에 따라 변하는 랜덤 수량의 일반 파편을 확정으로 생성한다. 그 뒤 별도의 보너스 드롭 판정을 한 번 가진다. 보너스 기본 확률은 15%이고, 플레이어의 잃은 체력 비율에 따라 선형으로 증가해 체력이 0에 가까울수록 최대 30%(기본의 2배)가 된다. 고등급 몬스터에 추가 독립 보상 판정을 더하지 않는다.
...
- 소형 힐팩은 회수 순간의 잃은 체력 25%를 회복한다. 일반 파편은 1층에서 3~7개를 무작위로 지급하고, 25층마다 범위 전체를 5개씩 올린다. 따라서 깊은 층도 등급보다 층수의 경제 성장을 크게 반영한다. 파편 꾸러미는 이 층수 기준값에 rare `1~2배`, unique `1~2.5배`, epic `1~3.5배`를 무작위로 적용하고 중앙값이 양끝보다 자주 나오게 하며, 꾸러미 결과만 5 단위로 정규화한다.
