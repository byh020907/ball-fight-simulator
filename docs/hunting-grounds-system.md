...
동굴 몬스터 14종은 모두 `monster` 태그와 하나의 희귀도 태그(`rarity:common`, `rarity:rare`, `rarity:unique`, `rarity:epic`)를 정의 데이터에 함께 둡니다. 사냥터 전투에서 플레이어가 승리하면 실제 편성된 일반 몬스터의 태그를 처치 통계에 누적합니다. 따라서 새 몬스터는 행동 구현과 함께 태그만 추가하면 기존 희귀도 처치 업적에 자동 포함됩니다.

중간 보스, 챔피언 난입, 최종 보스는 일반 몬스터 태그를 받지 않습니다. 챔피언 난입 승리는 별도 통계로 기록하고, 최종 보스는 스테이지 클리어 통계로 기록합니다. 이 구분은 보스 처치가 다수 몬스터 처치 업적을 부풀리지 않게 합니다.
...
| 챔피언 난입 | `champion_intrusion` | 강화 적 등장, 파편 보상 배율 1.5배 | 전투 준비 화면에서 `전투 시작` |
...
- 챔피언 난입: 강화 적과 파편 보상 배율은 전투 준비 화면에서 보여 주고, 같은 화면의 `전투 시작`으로 전투에 진입합니다.
...
| `champion_intrusion` | 강화 적과 파편 보상 1.5배를 전투 준비 화면에서 표시하고 `전투 시작` 뒤 전투 진입 |
