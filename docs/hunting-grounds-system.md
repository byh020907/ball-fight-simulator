...
| `electric` | 전기 마법사 | 20 | 사거리 안 대상에게 지속 전기 피해 | 하늘색 전기 연결선 |
...
| `barrier` | 방벽 볼 | 60 | 잠시 방어력을 높이는 방벽 | 파란 보호 고리 |
| `siphon` | 흡수 볼 | 70 | 대상 HP를 흡수 | 보라색 흡수 연결선 |
| `shard` | 파편 볼 | 80 | 부채꼴 파편 3발 발사 | 노란 파편 투사체 |
| `boomerang` | 부메랑 볼 | 91 | 적중 뒤에도 돌아오는 투사체 | 주황 귀환 공 |
| `splitter` | 분열 볼 | 92 | 여러 방향으로 분열 파편 발사 | 보라색 분열 탄막 |
| `jumper` | 도약 볼 | 93 | 목표를 향해 도약 | 커졌다 작아지는 몸체 |
| `laser` | 레이저 볼 | 94 | 충전 시 고정한 각도로 전장 벽까지 직선 레이저 발사 | 벽까지 이어지는 붉은 충전선과 굵은 레이저 |

코드의 단일 데이터 원본은 `src/hunting/huntingMonsters.js`의 `CAVE_MONSTERS`다. 전투 표시명·주력 전환 층·가중치·외형·기본 스탯을 수정할 때는 이 표와 함께 갱신한다.

레이저 몬스터는 충전 시작 시 목표 각도를 고정한 뒤 해당 방향의 전장 벽까지 충전선과 발사선을 유지한다. 피해 판정도 같은 벽 끝 범위 안에서만 적용된다. 힐러는 반경 안 아군 한 명에게만 초록 회복 빔을 연결해 자기 HP 100/s를 대상 HP 300/s로 전환한다. 힐러는 별도로 최대 HP의 5%/s를 자연 회복한다.
...
| 축복 | `boon` | 파편 획득 (8 + floor/10×3) | 결과 확인 후 전진 |
...
| 챔피언 난입 | `champion_intrusion` | 강화 적 등장, 파편 보상 배율 1.5배 | `전투 시작` 확인 후 전투 |
...
- 챔피언 난입: 강화 적과 파편 보상 배율을 확인한 뒤 `전투 시작`합니다.
...
- 구매와 리롤은 각각 실제로 사용한 뒤에만 남은 시간을 표시하며, 리롤은 제안 중인 상자의 등급을 즉시 다시 뽑습니다.
...
| `champion_intrusion` | 강화 적과 파편 보상 1.5배를 표시하고 `전투 시작` 뒤 전투 진입 |
