...
- 루팅 아이템은 기본적으로 플레이어 반지름의 4배 범위 안에서 자석으로 끌려온다. 일반 몬스터를 모두 처치해 승리하면 기존 결과 연출 안에서 약 1초 동안 남은 아이템을 전장 전체 자석으로 회수한 뒤 결과 흐름으로 넘어간다. 이 물리 보정은 Hero Orb의 자석 효과와 같은 공통 유틸리티를 쓰며, Hero Orb만 보유자의 초기 발사 유예와 능력 기반 범위 배율을 추가로 적용한다.
...
- 모든 전리품은 처치한 몬스터의 유효 이동 속도에 Hero Orb와 같은 1.2~1.5배 계수를 적용해 360도 무작위 방향으로 발사된다. 따라서 처치 위치에서 잠시 흩어진 뒤 기존 자석 회수 경로를 탄다.
