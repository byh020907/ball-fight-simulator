...
전기 마법사의 번개는 `src/effects/electricArc.js`가 시작점과 끝점 사이를 거리 기반 절점으로 나누고, 각 절점을 시간에 따라 수직으로 흔들어 그린다. 공통 렌더러는 외곽 광선·색 본선·흰 심선을 겹치므로, 다음 연결형 공격도 같은 아크 표현을 재사용할 수 있다.
