...
동굴 몬스터는 14종의 데이터 정의를 사용한다. 1층부터 추적 몬스터가 등장하고, 10층마다 새 행동 몬스터가 누적 해금된다. 최신 해금 몬스터는 높은 가중치로 해당 구간의 주력이 되며, 이전 몬스터도 계속 조우 후보에 남는다. 몬스터 행동은 플레이어 캐릭터 Ability를 재사용하지 않고 전용 행동으로 처리한다. 전기·회복·사슬·충격파·방벽·흡수·레이저 같은 연결선과 파동 이펙트는 행동을 읽게 하는 핵심 신호다.
