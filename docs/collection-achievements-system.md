...
숙련도는 다른 캐릭터에게 주는 크로스 지원 효과와 별개로, 다음 토너먼트에서 선택된 AI의 시작 경험치 진행도도 결정합니다. 미해금은 기본 Lv.1, BRONZE는 Lv.3, SILVER는 Lv.6, GOLD는 Lv.9이며 선택된 AI 전원은 같은 레벨의 기본 스탯과 행동 강화 tier를 받습니다. 플레이어 자신의 XP 레벨은 저장값을 그대로 사용합니다.
...
```js
{
    ...
    tier: "bronze",
    evaluate(context) {},
    reward: { type: "SHARDS", amount: 200 },
    grant(handler) {
        return handler.shards(200);
    }
}
```

`reward`는 표시용 불변 데이터이고 실제 지급은 정의의 `grant(handler)`가 `AchievementRewardHandler` 공개 메서드만 호출해 처리합니다. 판정과 지급을 분리해 같은 업적이 중복 지급되지 않게 합니다.
...
// localStorage key: bfs:player-profile:v1
{
    ...
    characterMastery: { levels: {} },
    tournamentChallenge: { levels: {} },
    experience: { currentXp: 0, byCharacter: {} },
    equipment: {
        inventory: [],
...
- `createTournamentReport()`는 토너먼트 준비 시 `reportId`를 한 번 생성하고 종료까지 유지합니다.
- ID 생성기는 테스트에서 주입할 수 있어야 합니다. 브라우저 기본 구현은 `crypto.randomUUID()`를 사용하고 미지원 환경에서는 시간과 난수를 조합한 ID로 대체합니다.
- `applyTournamentReport()`는 이미 `processedTournamentReportIds`에 있는 보고서를 받으면 아무 카운터도 변경하지 않고 `alreadyProcessed: true`를 반환합니다.
...
토너먼트 종료 시 `BattleApp._settleTournamentProgression()`이 보고서 반영, 업적 판정·보상, 숙련도·도전 진행을 순서대로 처리한 뒤 프로필을 한 번 저장합니다. 사냥터 업적도 같은 `_settleAchievements()` 경계를 사용합니다.
...
`evaluateAchievements()`는 신규 해금만 반환합니다. `BattleApp`은 해당 정의를 찾아 `grantAchievementReward()`로 보상을 지급하고 로그·토스트를 표시합니다. 판정은 보상 지급이나 UI를 직접 실행하지 않습니다.
...
- 전투 중 기록은 `matchReport.js`, 토너먼트 단위 확정은 `tournamentReport.js`가 소유합니다.
- 영구 프로필의 정규화와 저장은 `playerProfile.js`만 담당합니다.
- 업적 정의·판정·보상 지급은 각각의 컬렉션 모듈로 분리하며 UI는 `collectionViewModel.js`의 결과만 사용합니다.
- 새 업적이나 보상 타입을 추가할 때는 회귀 테스트, 게임 규칙, 도움말, 패치노트를 함께 갱신합니다.
...
사냥터 업적은 원정 중간에 바로 프로필을 바꾸지 않고, 귀환·패배·스테이지 클리어로 원정 결과가 확정될 때 한 번만 영구 통계에 병합합니다. 원정에서 이미 이긴 전투의 처치 기록은 패배하더라도 남지만, 전투 자체에서 패배한 적은 처치로 기록하지 않습니다.

일반 몬스터 정의는 단일 희귀도 필드 대신 `monster`, `rarity:common` 같은 `monsterTags` 배열을 가집니다. 처치 집계는 전투에서 승리한 실제 적 스펙의 태그를 세므로, 이후 새 몬스터에 태그를 더하면 업적 조건을 바꾸지 않고 같은 통계에 포함됩니다. 중간 보스·챔피언·최종 보스에는 일반 몬스터 태그를 붙이지 않아 희귀도 학살자 집계에 섞이지 않습니다.
