...
- 누적 환생 횟수는 정지·전투 모두에서 오라, 외곽선, 점선 문양, 불꽃으로 보인다. 불꽃은 공 표면의 넓은 호에 붙은 하나의 닫힌 실루엣이며, 정지 중에는 상단 외곽이 위로 솟고 이동 중에는 속도 반대쪽 외곽이 후류로 기울어진다. 기본 환생부터 길이·외곽이 눈에 띄게 일렁이며, 환생 단계가 높을수록 흔들림의 폭과 속도가 커진다. 뿌리는 현재 후류를 따르되 끝은 볼별 각도·각속도 감쇠 스프링으로 관성을 유지하므로 급회전에도 불꽃 실루엣 자체가 자연스럽게 휜다. 적색 외곽·주황 몸통·밝은 중심은 같은 실루엣을 겹쳐 표현하며, 이 비주얼은 물리 수치에 영향을 주지 않는다.
