...
- Grenade Ball은 상대 방향으로 2~4개의 수류탄을 산탄처럼 발사합니다(쿨타임 4.5초, 고정). 각 수류탄은 벽에 최대 2회 튕기며, 중심부일수록 짧은 퓨즈(0.25~0.8초)로 지연 폭발합니다. 폭발 데미지는 중심 20에서 가장자리 10까지 거리 비례 감소(기존 절반). 넉백이 크게 증가하여(600px/s, 0.5초) 맞은 상대가 뒤로 밀려 추가 폭탄에 연쇄 피해를 입을 수 있습니다. 센서 신관(140px, 0.2초 지연) 유지. 예측 조준/미스 스트릭/패시브 회피 제거.
