...
Lv.10 캐릭터는 준비 화면의 도감 상세 `환생` 탭에서 여는 강제 선택형 보상 팝업으로 후보 3장 중 하나를 고른다. 후보는 팝업을 열기 전에 저장되며 선택하기 전에는 남아 있으므로, 중단하거나 새로고침해도 다시 열면 같은 세 장을 본다. 실제 선택에서만 자신의 XP를 Lv.1로 초기화하고 환생 횟수를 1 올린다. 후보는 영구 기초 수치 보상, 다른 볼의 액션 카드, 패시브 카드가 같은 풀에서 비복원 추첨된다.
