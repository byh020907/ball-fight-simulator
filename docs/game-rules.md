...
- Grenade Ball은 상대 방향 120° 범위에서 2~4개의 수류탄을 순차 발사합니다(쿨타임 3.5초, 발사 간격 0.12초). 모든 수류탄은 800px/s로 날아가며, 첫발 퓨즈 0.6초에서 마지막발 2.0초로 선형 지연 폭발합니다. 벽에 최대 4회 튕깁니다. 폭발 데미지는 중심에서 `기본 공격력 × 2.5`, 가장자리에서 `기본 공격력 × 1.5`까지 거리 비례 감소하며, 넉백은 900px/s로 1.3초 동안 적용됩니다. 수류탄의 이동 경로가 상대 폭발권을 지나면 탄속에 비례해 남은 퓨즈를 20~85% 단축합니다. 기준 탄속 800px/s에서는 40% 단축됩니다.
