...
7. 사용자의 캐릭터가 우승하면 축하 메시지와 이번 숙련도 보상(보상 이름·실제 수치·다른 볼 적용 여부·다음 AI 시작 레벨)을 표시합니다.
8. 사용자의 캐릭터가 중간에 탈락하면 아쉬운 메시지와 최종 등수를 표시한 뒤, `확인`을 눌러 초기 화면으로 돌아갑니다. 탈락 뒤 남은 AI 대진은 자동 진행하지 않습니다.
