...
- 누적 환생 횟수는 정지·전투 모두에서 오라, 외곽선, 사각 셀 불꽃으로 보인다. 불꽃은 곡선 또는 닫힌 실루엣이 아니라 회전하는 사각 셀의 밀집된 흐름으로만 표현한다. 셀은 공 외곽 전체의 표면 앵커에서 시작해 수명 초반에는 캐릭터를 감싸고, 앵커 결합이 풀린 뒤에는 이동 방향과 무관하게 위로 타오른다. 공이 이동하면 결합이 느슨해진 셀이 이전 위치에 남아 자연스러운 후류가 생긴다. 셀은 밝은 중심에서 주황 몸통, 적색 끝으로 바뀌고 회전·크기·투명도가 수명과 흔들림에 따라 변한다. 셀 수는 환생 1회의 56개에서 10환생의 92개까지 정수 반올림 선형 보간하며, 크기와 흔들림도 같은 1~10 구간을 선형으로 강화한다. `source-over` 알파 합성을 사용해 과포화된 가산 발광을 피하며, 이 비주얼은 물리 수치나 저장 데이터에 영향을 주지 않는다.
