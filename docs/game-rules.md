...
사냥터의 처치 XP는 적이 떨어뜨린 여러 개의 둥근 황록색 오브를 실제로 주워 얻습니다. 등급·정예·보스일수록 오브 수와 XP 비중이 커지지만 일반 승리의 총 성장 속도는 기존 매치 XP 공식(`stageMultiplier = 1.0`)을 유지합니다. 승리하면 남은 오브를 전장 전체 자석으로 회수하고, 패배하면 이미 회수한 XP만 남습니다. 일반 승리 선택에는 이번 전투의 `+XP`가 짧게 표시되며, 패배·스테이지 클리어 결과는 공통 결과 시퀀스에서 XP 진행 패널을 먼저 보여 줍니다.
