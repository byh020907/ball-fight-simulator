...
- 누적 환생 횟수는 정지·전투 모두에서 오라, 외곽선, 점선 문양, 불꽃으로 보인다. 불꽃은 공 표면의 넓은 호에 붙은 하나의 닫힌 실루엣이며, 정지 중에는 상단 외곽이 위로 솟고 이동 중에는 속도 반대쪽 외곽이 후류로 기울어진다. 새로 나온 작은 시각 파티클은 처음에는 발화체 이동을 강하게 따라가다가 수명에 따라 결합이 풀리고, 이후 후류 추력·항력·난류로 움직인다. 이 플룸의 오래된 입자 흐름이 실루엣 끝 방향을 정하므로 급회전에도 불꽃 전체가 이전 궤적을 보존하며 자연스럽게 휜다. 적색 외곽·주황 몸통·밝은 중심은 같은 실루엣을 겹쳐 표현하고, 플룸은 가산 합성으로 열기를 보태며, 이 비주얼은 물리 수치에 영향을 주지 않는다.
