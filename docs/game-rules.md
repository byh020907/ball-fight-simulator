...
- Grenade Ball의 수류탄은 벽에 최대 2회 튕깁니다. 폭발 데미지는 중심 40에서 가장자리 20까지 거리 비례 감소. 빗나갈수록 퓨즈가 짧아져 더 빨리 터집니다. 쿨타임은 기본값 3.8초 기준 ±100% 범위에서 랜덤하게 결정됩니다. (0.3~1.3배). 예측 정확도(PREDICTION_FACTOR)가 0.18로 낮아 적의 이동을 거의 따라잡지 못하고, 센서 신관 범위도 140px로 축소되어 자주 빗나갑니다.
- Archer/Grenade Ball의 패시브 회피는 상대가 가까워질 때 옆 방향 impulse를 즉시 적용하고, 짧은 시간 동안 회피 방향을 유지합니다. Grenade Ball은 Archer와 동일한 패시브 회피를 가지되 사거리 260, 강도 0.5를 사용합니다.
