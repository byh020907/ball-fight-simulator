...
Spin Ball은 기본 쿨다운 `2.7초`마다 `applyTorque()`로 회전력을 가속한다. 회전력은 물리 각속도를 직접 대입하지 않고 토크와 각충격만으로 바뀌며, 충돌 접점 속도에 따른 기존 회전 피해 보정(최대 +60%)을 그대로 사용한다.

- 회전력은 `3.0rad/s`에서 링이 완성된다. 충돌 뒤에는 기본 64%를 보존하고, Lv.3 이후에는 82%를 보존한다.
- Lv.6 이후 회전력이 50% 이상이면 충돌 뒤 대상에게 회전 방향의 횡넉백을 적용한다.
- Lv.9 이후에는 회전력이 `2.7rad/s` 이상일 때 다음 가속이 오버스핀으로 바뀐다. 오버스핀은 회전력의 58%를 물리 각충격으로 소비하고 1.1초 동안 다음 충돌 피해를 45% 강화한다.
- 전투 카드에는 `회전력` 진행률을, 오버스핀 대기 중에는 `오버스핀`을 표시한다.
