...
Lv.10 캐릭터는 준비 화면의 도감 상세 `환생` 탭에서 후보 3장 중 하나를 골라 자신의 XP만 Lv.1로 초기화하고 환생 횟수를 1 올린다. 후보는 스탯 카드가 높은 비중, 다른 볼의 서브 스킬 카드가 낮은 비중으로 제시되며 같은 후보 안에서 중복하지 않는다.
...
- 누적 환생 횟수는 정지·전투 모두에서 오라, 외곽선, 점선 문양, 불꽃으로 보인다. 이 비주얼은 물리 수치에 영향을 주지 않는다.
