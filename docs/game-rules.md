...
Spin Ball은 충돌하지 않은 시간 동안 `4.5초`를 기준으로 회전 충전을 연속으로 쌓는다. 충전 상태는 목표 각속도 `10rad/s`로 변환되고, 실제 회전은 `applyTorque()`로 그 목표를 따라간다. 충돌 직후에는 각속도를 직접 대입하지 않고 각충격으로 낮춘다. 충돌 접점 속도에 따른 기존 회전 피해 보정은 그대로 사용하며, 만충 시 기존 상한인 `+60%`까지 강해진다.
