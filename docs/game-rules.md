...
7. 사용자의 캐릭터가 우승하면 모바일 결과 순서에서 경험치 → 숙련도 승급 결과(보상 이름·실제 수치·다른 볼 적용 여부·다음 AI 시작 레벨) → 기존 축하 메시지를 차례로 표시합니다. 카드 오른쪽 세로 탭의 `》`로 중간 단계를 넘기며, 마지막 축하 단계에서는 같은 탭의 `확인`을 눌러 초기 화면으로 돌아갑니다.
8. 사용자의 캐릭터가 중간에 탈락하면 경험치와 아쉬운 메시지·최종 등수를 같은 결과 순서로 표시한 뒤, 카드 오른쪽 마지막 탭의 `확인`으로 초기 화면으로 돌아갑니다. 탈락 뒤 남은 AI 대진은 자동 진행하지 않습니다.
