...
토너먼트 전투에서만 현재 속도 직선으로 다음 벽 충돌을 짧게 예측한다. 벽의 안쪽 법선을 기준으로 한 입사각이 `25°` 이하이고 도달까지 `0.25초` 이하일 때, 예상 충돌점에서 벽 안쪽 `55px` 위치에 길이 `120px`, 두께 `16px`의 임시 다각형 경사면 하나를 만든다. 긴 변은 벽 법선에서 시드 난수로 고른 `±25~45°`만큼 기울며, 같은 경기의 같은 상태는 같은 방향을 재현한다.

- 경사면은 기존 사냥터 다각형 지형과 같은 충돌·나무 재질 계약을 사용한다. 충돌 반사와 회전 충격은 공통 물리 응답이 계산하며, 경사면 시스템은 `velocity`나 `angularVelocity`를 직접 바꾸지 않는다.
- 벽 그 자체의 형상·재질·충돌 규칙은 바꾸지 않는다. 경사면은 실제 공 충돌 직후 제거되며, 충돌하지 않으면 `0.7초` 뒤 제거한다.
