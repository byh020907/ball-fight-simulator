...
- Archer Ball은 조준 완료 시 상대의 예측 위치(현재 위치·속도와 화살 속도)로 요격 지점을 계산해 발사합니다. 일직선 조준선으로 조준하는 동안 dash 원으로 예측 명중 지점을 표시합니다. 발사 뒤 유도는 없습니다. Lv.3 이후: 예측 조준으로 업그레이드됩니다. Lv.6 이후: 0.6초 windup 후 70% 단발 / 30% 연속 2발(0.12s 재조준). Lv.9 이후: 50% 확률로 critChance × 2(최대 100%)의 독립 크리티컬 roll.
