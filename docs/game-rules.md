...
```
받는 피해 = max(1, 들어온 피해 × 방어력배율)

방어력배율 = 100 / (100 + totalDefense)
  - totalDefense = baseDefense × (1 + 스탯포인트/100) × 밸런스배율 + 능력보정(defense)
  - 능력보정: Eater 피스트 중 +3 (flat)
```

예: 방어력 4의 Eater가 10 피해를 받으면 `10 × 100/104 ≈ 10` (소수점 반올림 전). 피스트 중 방어력 7이면 `10 × 100/107 ≈ 9`.
