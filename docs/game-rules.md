...
- 누적 환생 횟수는 정지·전투 모두에서 오라, 외곽선, 사각 셀 불꽃으로 보인다. 불꽃은 곡선 또는 닫힌 실루엣이 아니라 회전하는 사각 셀의 밀집된 흐름으로만 표현한다. 셀은 공 외곽 전체의 표면 앵커에서 시작해 수명 초반에는 캐릭터를 감싸고, 수명에 따라 앵커 결합이 풀리면 공통 후류 추력·항력·난류로 합류한다. 정지 중에는 위로, 이동 중에는 속도 반대쪽으로 흐르며, 오래된 셀의 흐름은 급회전 직후에도 이전 궤적을 남긴다. 셀은 밝은 중심에서 주황 몸통, 적색 끝으로 바뀌고 회전·크기·투명도가 수명과 흔들림에 따라 변한다. `source-over` 알파 합성을 사용해 과포화된 가산 발광을 피하며, 환생 단계가 높을수록 셀 수·크기·흔들림을 높인다. 이 비주얼은 물리 수치나 저장 데이터에 영향을 주지 않는다.
