...
새 Lv.10 도달은 전투 결과를 `확인`해 준비 화면으로 복귀한 뒤 한 번만 알리고, 안내의 주 동작은 해당 캐릭터의 도감 상세 `환생` 탭을 연다. 이 탭에서 후보 3장을 만들거나 저장된 후보를 인라인으로 확인해 하나를 고른다. 후보는 선택 전에 저장되며 탭을 닫거나 새로고침해도 다시 열면 같은 세 장을 본다. 실제 선택에서만 자신의 XP를 Lv.1로 초기화하고 환생 횟수를 1 올린다. 후보는 영구 기초 수치 보상, 다른 볼의 액션 카드, 패시브 카드가 같은 풀에서 비복원 추첨된다.
...
- 다음 토너먼트에서 선택된 AI 전원은 현재 플레이어 숙련도에 따라 같은 시작 경험치 진행도를 사용합니다: 미해금은 기본 Lv.1, BRONZE는 Lv.3, SILVER는 Lv.6, GOLD는 Lv.9입니다. 이 진행도는 AI 기본 스탯과 행동 강화 tier에 함께 적용되며 사냥터나 프로필 저장 XP는 바꾸지 않습니다.
- 시작 전 플레이어 패널은 현재 토너먼트 단계를 `첫 도전` 또는 `BRONZE`/`SILVER`/`GOLD`로, 바로 옆에 해당 AI의 시작 레벨을 함께 표시합니다.
