...
토너먼트 전투에서만 현재 속도 직선으로 다음 벽 충돌을 짧게 예측한다. 벽의 안쪽 법선을 기준으로 한 입사각이 `25°` 이하이고 도달까지 `0.25초` 이하일 때, 예상 충돌점의 벽선에 길이 `120px`인 변을 붙인 임시 직각삼각형 하나를 만든다. 삼각형은 벽에서 경기장 안쪽으로 `55px` 깊이까지 뻗고, 시드 난수로 고른 위·아래 방향 중 한쪽에 직각 꼭짓점을 둔다.
