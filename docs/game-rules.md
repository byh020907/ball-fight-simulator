...
- 누적 환생 횟수는 정지·전투 모두에서 오라, 외곽선, 점선 문양, 불꽃으로 보인다. 불꽃은 공 표면의 넓은 호에 붙은 하나의 닫힌 실루엣이며, 정지 중에는 상단 외곽이 위로 솟고 이동 중에는 속도 반대쪽 외곽이 후류로 기울어진다. 방향은 볼별 시각 상태에서 짧게 보간해 급회전해도 즉시 꺾이지 않는다. 적색 외곽·주황 몸통·밝은 중심은 같은 실루엣을 겹쳐 표현하며, 이 비주얼은 물리 수치에 영향을 주지 않는다.
