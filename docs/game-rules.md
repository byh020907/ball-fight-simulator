...
사냥터의 처치 XP는 적이 떨어뜨린 여러 개의 둥근 황록색 오브를 실제로 주워 얻습니다. 등급·정예·보스일수록 오브 수와 XP 비중이 커지고, 한 알의 XP가 클수록 오브도 커져 가치 차이를 전장에서 읽을 수 있습니다. 일반 승리의 총 성장 속도는 기존 매치 XP 공식(`stageMultiplier = 1.0`)을 유지합니다. 승리하면 남은 오브를 전장 전체 자석으로 회수하고, 패배하면 이미 회수한 XP만 남습니다. 일반 승리 선택과 패배·스테이지 클리어 결과 모두 공통 XP 보상 패널의 진행 바로 이번 전투의 성장 결과를 보여 줍니다.
