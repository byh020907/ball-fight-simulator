...
### 완료: 교차 게임 재사용 game-kit 분리

- `Vector2`의 단일 구현과 물리 capability를 `src/game-kit/physics/`로 이동했습니다.
- 정적 Canvas 캐시·화면 가독성 계산·일반 경로 연출, 컬렉션 셔플, 화면 깨우기 잠금을 책임별 하위 폴더로 모았습니다.
- `src/game-kit/index.js` 하나를 다른 게임의 공개 진입점으로 두고, 내부 상대 import가 game-kit 경계 밖으로 나가지 못하게 구조 테스트로 고정했습니다.
- 기존 코드는 `src/core.js`와 `src/physics/index.js`의 호환 re-export를 통해 같은 `Vector2` 클래스 정체성을 유지합니다.
- 변경 전후 전투 시뮬레이션은 같은 10개 시점 해시를 유지합니다.
