...
- `HuntingManager`는 이벤트 전이 결과를 전투 시작, 선택 UI, 상인 UI로 연결하는 조정자입니다. 이벤트 타입별 보상·HP·전리품 로직을 직접 소유하지 않습니다.
...
이벤트 결과·전투 준비 화면은 같은 카드 구조를 사용한다. 카드 본문은 `minmax(0, 1fr)` 가용 행 안에서만 스크롤하고, 계속·전투 시작 같은 행동은 별도 `auto` 행에 둔다. 오버레이와 카드는 부모의 가용 크기를 grid/flex로 나누므로 기기별 고정 폭·높이 보정으로 행동 버튼을 살리지 않는다.
