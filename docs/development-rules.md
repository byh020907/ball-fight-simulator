...
HeroAbility:               1초 성장 스택 충전(최대 5),
                           ...
                           owner별 활성 코어 제한(5개), 자석·공명 조각 상태 소유
HeroOrb entity:            이동, 충돌 대상 판정, owner/상대 구분,
                           0.16초 직접 회수 유예와 8초 수명,
                           apply() 반환값 기반 획득 텍스트 피드백(spawnActionText)
HeroResonanceEffect:       조각별 0.1초 곡선 추적·×0.20 피해,
                           정확히 5조각일 때 Heroic Burst 1회
...
- Lv.3 자석은 `HeroAbility.getOrbAttraction()`이 성장 코어에만 반환하며, 순간 이동이나 전역 회수로 바꾸지 않는다.
- Lv.6 공명 조각은 실제 스탯 적용 성공 때만 저장하고 최대 5개다. 다음 적대 전투원 충돌에서 `HeroResonanceEffect`가 같은 대상을 추적한다.
- Lv.9 Heroic Burst는 정확히 5조각의 다섯 번째 적중에만 한 번 실행한다.
