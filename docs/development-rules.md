...
- **정적 표면(벽/terrain) 충돌**은 `applyCollisionResponse(body, normal, contactPoint, preCollisionVelocity, options)`를 통해 동일한 `_resolveContactImpulse` 공통 solver를 호출합니다. 구현에서는 `bodyA=null`을 정적 표면으로 두고 `bodyB=body`를 동적 body로 전달하므로, denominator에는 bodyB의 질량과 관성 기여만 포함됩니다. 호출자는 위치 보정(clamp)과 contact normal/contactPoint/preCollisionVelocity 계산까지만 담당합니다. 반발 계수 기본값은 0.92(terrain), 벽은 1.0, 접선 마찰 계수 기본값은 0.03입니다.
