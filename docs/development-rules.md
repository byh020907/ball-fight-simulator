...
- **벽 충돌**은 `simulation.js`의 `_reflectX`/`_reflectY`에서 위치 clamp 후 `applyCollisionResponse(entity, normal, contactPoint, preVel, { restitution: 1, tangentialFriction: 0.08 })`를 호출합니다. `angularFactor`는 기본값 1 (물리적으로 정확한 angular impulse)을 사용합니다.
- **terrain 충돌(circle/polygon)**은 `terrainCollision.js`/`CollisionShape.js`에서 위치 보정 후 `applyCollisionResponse(entity, normal, contactPoint, preVel, { restitution: 0.92, tangentialFriction: 0.08 })`를 호출합니다.
