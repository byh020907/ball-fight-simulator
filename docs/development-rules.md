...
- `SESSION-HANDOFF.md`는 현재 계약 요약, 열린 리스크, 아직 문서에 완전히 흡수되지 않은 활성 결정을 기록합니다.
- `docs/decision-history.md`는 현행 문서에 흡수됐거나 폐기·대체된 결정의 전체 배경을 보존합니다.
...
사용자가 `문서 현행화`, `문서 정합`, 또는 같은 의미의 요청을 하면 아래 절차를 적용합니다.

1. 현재 코드, 테스트, 최근 `SESSION-HANDOFF.md`의 L1/L2 결정을 현행 동작의 근거로 삼습니다.
2. 각 문서를 `그대로 유지`, `부분 수정`, `레거시 이동`으로 분류하고 분류 근거를 확인합니다.
3. 경로·이름·수치·상태·책임 설명을 일부 고치면 현재 구현을 정확히 설명할 수 있는 문서는 `docs/`에 유지하고 즉시 정합합니다.
4. 문서의 목적, 구조, 전제, 본문 대부분을 다시 써야 현행 계약으로 사용할 수 있는 경우에만 `.legacy/docs/`로 이동합니다.
5. 작성일이 오래됐거나 구현이 완료됐거나 다른 문서와 일부 중복된다는 이유만으로 레거시 처리하지 않습니다.
6. 레거시 이동 시 `.legacy/README.md`의 보관 목록과 이유를 갱신하고, 현행 문서의 링크를 끊지 않습니다.
7. 완료 전 Markdown UTF-8, 로컬 링크, 존재하지 않는 코드 경로, 남은 `미구현`·`예정` 표현을 검사합니다. 코드 계약을 설명하는 문서를 바꿨다면 전체 테스트·문법·포맷 검사도 실행합니다.
8. 활성 결정의 내용과 이유가 현행 문서에 완전히 흡수됐으면 전체 항목을 `docs/decision-history.md`로 이동합니다. 일부 반영, 열린 리스크, 후속 작업이 남아 있으면 `SESSION-HANDOFF.md`에 유지합니다.

`.legacy/` 문서는 결정 배경을 보존하는 참고 기록이며 현행 구현 계약으로 인용하지 않습니다.

### 결정 기록 수명주기

- 새 L1/L2 결정은 먼저 `SESSION-HANDOFF.md`에 기록합니다.
- 코드와 관련 문서에 결정의 내용뿐 아니라 이유·제약까지 반영됐을 때만 흡수 완료로 판정합니다.
- 흡수 완료된 결정은 삭제하지 않고 `docs/decision-history.md`로 이동합니다.
- 폐기·대체된 결정은 전체 이력에 보존하고 어떤 결정이 대체했는지 명시합니다.
- 전체 이력은 `.legacy/`가 아닙니다. 과거에 유효했던 결정의 검색 가능한 제도 기록입니다.
- 새 대화 시작 시와 L1/L2 결정 추가 직후 활성 인계의 크기를 측정합니다. 200줄 이상, 활성 L1/L2 결정 20개 이상, UTF-8 32 KiB 이상 중 하나라도 충족하면 실제 측정값과 함께 같은 세션에서 한 번만 정리를 선제 제안합니다.
- 선제 제안은 현재 작업을 막지 않으며, 사용자 승인 전에는 결정을 자동으로 이동하지 않습니다. 승인 후에도 내용·이유·열린 리스크가 현행 문서에 완전히 흡수된 결정만 전체 이력으로 이동합니다.
