...
토너먼트 도전 단계는 숙련도와 별도 저장한다. 도전 0/1/2/3은 각각 AI Lv.1/Lv.3/Lv.6/Lv.9를 시작점으로 쓰고, 토너먼트 우승 뒤 다음 단계로 한 칸 올라간다. 숙련도는 도전을 완료한 시점의 단계로 승급을 판정하지만, 환생에서는 유지된다. 반대로 환생한 캐릭터의 도전 단계만 0으로 돌아가므로 GOLD 숙련도여도 다음 토너먼트는 Lv.1 AI부터 다시 시작한다.

시작 전 플레이어 패널과 토너먼트 첫 매치 전 도입 오버레이는 모두 현재 도전 단계로 문구와 AI 시작 레벨을 계산한다. 도전 0은 `첫 도전 · 상대 Lv.1 시작`이고, 도전 1/2/3만 각각 `BRONZE · 상대 Lv.3 시작`, `SILVER · 상대 Lv.6 시작`, `GOLD · 상대 Lv.9 시작`이다. 보유 숙련도는 도감과 교차 지원에 유지되지만 이 토너먼트 문구의 원본이 아니다. 첫 매치는 이 정보를 보여 준 다음 VS 대진으로 넘어간다. 우승 결과는 경험치 → 숙련도 승급 결과 → 축하 요약 순서이며, 화면 중앙 카드의 오른쪽 경계에 겹친 작은 세로 탭의 `》`로 중간 단계를 넘기고 마지막 단계에서는 같은 탭이 `확인`으로 바뀐다. 숙련도가 승급된 단계에는 원본 볼·새 등급·보상 이름·실제 수치·적용 범위·다음 AI 시작 레벨을 표시한다. 도전 표시는 `getCharacterChallengeLevel()`과 `getTournamentOpponentExperienceLevel()`을 그대로 사용하므로 실제 선택 AI의 시작 경험치 진행도와 분리되지 않는다.
...
모든 현행 수치는 퍼센트다. Bat Ball의 과거 액션 HP 비용 감소 값만 저장 호환을 위해 남아 있으며 현재 전투에는 적용되지 않는다.
...
| Bat Ball       | 퇴역 액션 HP 비용 감소 (현재 비활성)             | 0.03%p | 0.06%p | 0.10%p |
...
- 현재 플레이 캐릭터가 Elementalist이면 자기 숙련도는 제외한다. 다른 숙련도와 동일하게 `statModifiers.defense`에 가산한 뒤 전투 스펙에 적용한다.
- 잠긴 동안에도 전체 로스터 숙련도 완료 업적의 요구 대상에는 포함되지만 이름과 효과 상세은 도감에 노출하지 않는다.
