...
시작 전 플레이어 패널은 숙련도 등급과 현재 도전 단계에서 계산한 AI 시작 레벨을 함께 보여 준다. 미해금은 `첫 도전 · 상대 Lv.1 시작`, 보유 숙련도는 등급을 유지한 채 실제 다음 상대 레벨을 표시한다. 토너먼트 첫 매치에서 클릭 액션 카드를 고른 뒤에도 같은 정보를 잠깐 오버레이로 보여 준 다음 VS 대진으로 넘어간다. 우승 결과는 경험치 → 숙련도 승급 결과 → 축하 요약 순서이며, 화면 중앙 카드의 오른쪽 경계에 겹친 작은 세로 탭의 `》`로 중간 단계를 넘기고 마지막 단계에서는 같은 탭이 `확인`으로 바뀐다. 숙련도가 승급된 단계에는 원본 볼·새 등급·보상 이름·실제 수치·적용 범위·다음 AI 시작 레벨을 표시한다. 표시는 `getCharacterMasteryLevel()`과 `getTournamentOpponentExperienceLevel()`을 그대로 사용하므로 실제 선택 AI의 시작 경험치 진행도와 분리되지 않는다.
