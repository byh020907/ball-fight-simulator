...
현재 `BattleApp`은 토너먼트 종료 후에도 `playerFighterId`를 유지하므로 같은 페이지에서 다시 시작하면 같은 캐릭터가 계속 선택됩니다. 연계 시스템의 수집 목적과 맞지 않으므로 다음 흐름으로 변경합니다.

1. 토너먼트 종료 화면에서는 현재 결과를 그대로 유지합니다.
2. 사용자가 `새 토너먼트 준비`를 누르면 새 대표 캐릭터를 무작위로 뽑습니다.
3. 로스터가 2명 이상이면 직전 대표 캐릭터는 한 번 제외해 연속 중복을 방지합니다.
4. 플레이어 스탯 배분을 0으로 초기화합니다.
5. 새 캐릭터의 자동 연계 효과를 계산하고 설정 화면으로 돌아갑니다.
6. 사용자가 새 포인트를 모두 배분한 뒤에만 토너먼트를 시작합니다.

`새 토너먼트 준비` 클릭과 실제 `토너먼트 시작`은 같은 동작이 아닙니다. 완료된 토너먼트 상태에서 첫 클릭이 새 대진을 즉시 시작하면 안 됩니다.
...
11. `docs/game-rules.md`, `src/help-content.js`, `src/patch-notes.js` 갱신
