...
Gunner가 cooldown ready인 focal 수동 플레이어이고 command resource가 있으면 다음 자동 burst 전에 0.8초의 `트레이서 라인` 입력창을 연다. 유효 release는 이동 발사를 만들지 않는 payload-only 명령이며, 기존 6~12발·0.05초 간격·발수 기반 피해·탄속·무작위 난사 규칙을 유지한다.
