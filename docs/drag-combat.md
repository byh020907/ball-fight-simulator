...
- 차징은 공격자 둘레의 진행 링, 어두운 외곽 레일, 이동하는 점선 코어·방향 셰브론과 끝점 조준점으로 표시한다. 평상시는 빨강, 조기 종료 가속은 호박색 `돌진 가속`으로 구분한다. 플레이어 드래그와 마찬가지로 실제 발사 뒤에는 공통 꼬리·전방 끝표식·외곽 링·`돌진` 라벨을 만들지 않고 공 자체의 이동만 보여 준다.
