...
클릭 같은 UI 이벤트 핸들러는 컴포넌트의 `Alpine.data()` 메서드에 둡니다. 컴포넌트는 `Alpine.store("uiManager").invokeGameAction()`을 통해 `BattleApp`의 공개 메서드를 호출하고, 게임 규칙/사냥터 진행/토너먼트 시작 같은 도메인 로직을 store 객체나 템플릿 표현식 안에 직접 넣지 않습니다. `window` 전역으로 액션 브리지를 노출하거나 템플릿에서 접근하지 않습니다.
...
- **UI 핸들러 소유**: `@click="doThing()"`처럼 컴포넌트 메서드를 호출하고, 그 메서드에서 `Alpine.store("uiManager").invokeGameAction()`을 통해 app/game 공개 핸들러를 호출. `uiManager`에는 단일 `gameActionBridge`만 설치하며 `_actions` 같은 콜백 레지스트리는 두지 않음
...
- 결과 확인과 사냥 진행처럼 같은 캔버스 위에 표시되더라도, 상태 전이·액션·레이아웃이 다른 화면은 별도 컴포넌트로 둔다.
- `game-overlay`는 일반 알림과 결과 시퀀스, `hunting-overlay`는 사냥터 이동·선택·상점·상자·전투 준비·미확보 전리품 HUD를 소유한다.
- `BattleApp`은 현재 게임 모드에 따라 하나의 오버레이만 보이게 전환한다. 결과 시퀀스를 표시할 때는 사냥터 오버레이를 먼저 초기화한다.
- 별도 오버레이가 공통 하단 버튼의 안전 영역을 사용해야 하면 전역 레이아웃 선택자는 두 컴포넌트의 루트를 함께 대상으로 삼되, 카드 크기 제한은 각 컴포넌트의 scoped CSS가 소유한다.
