...
> 토너먼트의 선택형 클릭 액션은 유지한다. `압박`·`이탈`은 사냥터 전용 전장 가장자리 버튼이며 HP를 소모하지 않아 캔버스 클릭 액션과 중복 발동하지 않는다. 토너먼트에서는 버튼이 표시되지 않고 호출도 거절된다.
...
3. 선택된 액션이 해당 토너먼트의 액션으로 고정
4. **토너먼트 최초 매치에서 한 번 선택, 토너먼트 종료까지 변경되지 않음**
5. 카드 3장은 중복 없이 뽑는다. 풀 크기가 3 미만이면 풀 전체를 보여주고 선택만 받는다.
6. **모든 캐릭터는 모든 액션을 사용할 수 있다.** 캐릭터 타입(원거리/근접)에 따른 액션 제한은 없다.
...
| **충격파**    | 상시           | 반경 250px 내 대상을 밀쳐냄 (중심 최대 1200 knockback, 0.3s 방향 고정). 벽 충돌 시 충격파 데미지 | 중간 (위치 싸움)  | 1.2%               |
| **흡혈**      | 상시           | 클릭 시 HP를 흡혈하여 회복. 효과는 일정 시간 지속                                       | 낮음 (위기 시)             | 2.0%               |
| **회피**      | 상시           | 클릭 시 현재 진행 방향 좌/우 90도 중 하나로 즉시 대시 회피 + 0.4초 속도 +30%            | 중간 (예측 회피)           | 1.0%               |

---

## AI 액션 규칙

- **AI도 플레이어와 동일한 액션 풀에서 3개를 무작위로 할당**받는다. 캐릭터 타입 제한 없이 모든 액션이 할당될 수 있다.
- **AI의 액션은 토너먼트 시작 시 할당되고 절대 변하지 않는다.** 인간 플레이어의 선택-고정 패턴과 동일하다.
- **`canAIUse(sim, fighter, opponent, hpRatio, distance)`** — AI가 이 액션을 사용할 전략적 조건을 판단하는 함수. 각 Action 클래스가 자신의 발동 조건을 직접 소유한다.
  - 원거리 캐릭터: 시간왜곡/투사체방어 선호, 돌진 기피
  - 근접 캐릭터: 돌진/카운터/충격파 선호
  - 저HP: 흡혈/회피 선호
- AI는 `canAIUse`가 false인 액션도 할당은 받을 수 있다. 단, 실제 사용은 `evaluate()`에서 조건 충족 시에만 이뤄진다.
- AI는 HP 50% 미만이거나 HP 소모 후 20% 미만이면 액션을 사용하지 않는다. 사용 후 15초 쿨다운이 적용된다.
- AI 액션 시스템은 `src/simulation/aiActionController.js`에 구현되어 있다.
    ...
    apply(sim, playerBall) {
        playerBall.actionContext.setEffect(this.id, {
            ...
            onFighterCollision: (owner, opponent, outgoingDamage, incomingDamage, simulation) => {
                opponent.takeDamage(Math.round(incomingDamage * COUNTER_REFLECT_RATE), owner, "Counter");
...
더 이상 충돌 임박 여부를 미리 체크하지 않는다. 사용자가 카운터를 누르면 무조건 HP를 소모하고 0.20초짜리 충돌 window가 열린다. 이 window 안에 충돌하면 `ActionContext.onFighterCollision()`이 상대가 나에게 줄 충돌 피해를 상대에게 반사하고, 내 충돌 피해는 0으로 조정한다. 충돌하지 못하면 HP만 소모된 채 만료된다.

### 투사체 방어 판정

더 이상 투사체 접근 여부를 미리 체크하지 않는다. 사용자가 투사체 방어를 누르면 무조건 HP를 소모하고 0.3초짜리 투사체 방어 window가 열린다. 이 window 안에 투사체 공통 hit 경로가 `ActionContext.onProjectileDamage()`를 호출하면 피해량을 75% 경감하고 "투사체 방어!" 텍스트를 표시하며, 실제로 지불한 HP 비용을 회수한다. 일반 충돌/근접 피해는 투사체 방어로 줄어들지 않는다.
...
- 투사체 방어는 모든 매치업에서 항상 사용 가능 (상시 발동 + 0.3초 window)으로 무용지물 문제 해결
...
| 충격파 | 1.2% | 250px 내 대상 밀침 (1200 knockback, 0.3s) + 벽꽝 데미지 |
| 흡혈 | 2.0% | HP 흡혈 회복 |
| 회피 | 0.8% | 좌/우 90도 대시 회피 + 0.4s 속도 +30% |
