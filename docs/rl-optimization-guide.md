...
const CONFIG = {
    ...
    logInterval: 100, characterIds: ["all"], actionIds: ["all"],
    opponentMode: "fixed", fixedOpponent: "rage",
};
...
| `opponentMode` | `fixed` | `fixed` 또는 `random` 상대 선택 |
...
기본 실행은 로스터의 모든 캐릭터와 액션 풀의 모든 액션을 곱해 학습합니다.

```bash
# 전체 12캐릭터 × 8액션 = 96 조합
node scripts/rl/train.mjs

# 빠른 스모크 테스트: 앞 2개 조합만, 각 4판
RL_MAX_COMBOS=2 RL_EPISODES=4 RL_BATCH_SIZE=2 RL_MINI_BATCH_SIZE=2 RL_LOG_INTERVAL=2 RL_EVAL_EPISODES=3 node scripts/rl/train.mjs

# 부분 학습
RL_CHARACTERS=dash,eater RL_ACTIONS=rush,life_steal node scripts/rl/train.mjs

# 랜덤 상대 커리큘럼
RL_OPPONENT_MODE=random node scripts/rl/train.mjs
```
...
### 8.3 2단계: 투사체 액션 (고정 상대: Gunner, Archer, Grenade)
...
## 10. 현재 구현 상태 (2026-07-02 업데이트)

### 10.1 특성 차원: 16차원

기존 14차원에서 캐릭터 정체성 2차원 추가:

| 인덱스 | 이름 | 설명 |
|---|---|---|
| 14 | myCharIdx | `ALL_CHAR_IDS.indexOf(fighter.id) / (len-1)` |
| 15 | oppCharIdx | `ALL_CHAR_IDS.indexOf(opponent.id) / (len-1)` |

### 10.2 보상 설계: 3축 구조

```
reward = 승패(±1)
       + (액션으로 깎은 상대HP / 상대maxHp) × hpWeight
       - (액션 후 내가 잃은 HP / 내maxHp) × survivalWeight
       - 액션사용횟수 × penalty
```

**핵심**: 액션 직전→직후 HP 차이만 측정. 충돌 등 수동 피해는 제외.

### 10.3 액션별 가중치 (`scripts/rl/train.mjs`)

```js
const ACTION_WEIGHT_MAP = {
    shockwave:        { hp: 0.5, survival: 0.1,  penalty: 0.02 },  // 공격형
    evade:            { hp: 0.1, survival: 0.5,  penalty: 0.01 },  // 방어형
    counter:          { hp: 0.2, survival: 0.4,  penalty: 0.01 },  // 방어형
    projectile_guard: { hp: 0.1, survival: 0.5,  penalty: 0.01 },  // 방어형
    rush:             { hp: 0.15, survival: 0.15, penalty: 0.02 }, // 유틸형
    time_warp:        { hp: 0.2, survival: 0.2,  penalty: 0.02 },  // 유틸형
    life_steal:       { hp: 0.3, survival: 0.2,  penalty: 0.02 },  // 회복형
    endure:           { hp: 0.1, survival: 0.35, penalty: 0.02 },  // 생존형
};
```

환경변수 `RL_HP_WEIGHT`, `RL_SURVIVAL_WEIGHT`, `RL_ACTION_PENALTY` 로 전체 override 가능.

### 10.4 훈련 vs 게임 결정 간격

| | 훈련 | 게임 |
|---|---|---|
| 평가 빈도 | **0.5초**(30프레임)마다 | **매 틱**(60fps) |
| 사용 후 쿨다운 | 0.5초 | **0.5초** |
| 이유 | 동일 데이터 스팸 방지, 효율 | 반응성 최대화 |

의도적 비대칭: 훈련은 frame-skip으로 효율적 학습, 게임은 매 틱 평가로 즉시 반응.

### 10.5 모델 저장/로드

- **저장**: `models/{actionId}/{charId}.json` — TF.js LayersModel + normalizer + metadata
- **로드(게임)**: `AIActionController.loadRlPolicy(charId)` → `fetch()` → `RLPolicy.fromJson()`
- **로드(Node.js)**: `benchmark.mjs`에서 `loadPolicy()` 사용
- **웜업**: 로드 직후 더미 추론 1회로 JIT 컴파일 렉 방지

### 10.6 벤치마크 + 스팸 감지

`scripts/benchmark.mjs`: 액션 없음 베이스라인 vs RL 모델 사용 승률 비교.

추가 지표:
- **Accept%**: 전체 결정 중 "사용" 비율 (90%+ → 스팸)
- **ProbMean**: 평균 예측 확률
- **Spam**: 🚨스팸(>90%) / ⚠️의심(>50%) / ✅전략

### 10.7 게임 통합

- **AI 의사결정 로그**: 게임 내 UI 로그에 `이름: 액션 O(78%)` / `X(32%)` / `⏳(쿨다운)` 표시
- **이펙트**: `_executeAction()` 에서 explosion+pulse+sound 통합 (플레이어/AI 동일)
- **Time Warp**: 시전자별 독립 타이머 (양쪽 사용 시 서로 느려짐, 중첩 없음)
- **pendingActions**: 단일 슬롯→큐로 변경 (여러 AI 동시 액션 누락 방지)

### 10.8 npm 스크립트

```bash
npm run train       # node scripts/rl/train.mjs
npm run benchmark   # node scripts/benchmark.mjs
npm start           # 게임 서버
npm test            # 회귀 테스트
```

### 10.9 벤치마크 결과 (2026-07-02, 96조합, 액션별 가중치 적용)
...
주요 발견: 시간 왜곡 최강 (거의 모든 캐릭터 +20~60%), Eater 대반전 (구버전 0.3%→신버전 +20~40%), Gunner만 유일하게 전체 마이너스.
```
