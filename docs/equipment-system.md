...
- 카드에는 Canvas `x-equipment-icon-tag`, 정식명, 보유 수량, 원시 스탯을 보이고, 선택한 카드의 상세 영역에서 결과에서 재료로 내려가는 아이콘 조합 트리, 재료 `보유/필요`, 조합 비용과 결과 스탯을 확인한다. 조합 헤더의 `상위 조합`은 선택 장비를 직접 재료로 쓰는 장비만 템플릿 선언 순서로 보여 주고, 아이콘을 누르면 기존 선택 흐름으로 그 상세를 연다. 상위가 없으면 `최종 장비`를 표시하며, 상위 행과 하위 트리는 각각 독립 가로 스크롤한다. 역방향 레시피 관계와 카드 데이터는 `equipmentPresentation.js`가 소유하고, 트리는 정적 레시피 조회와 보유 수량 조회만 받는 읽기 전용 presentation으로 실제 조합 판단·저장·재료 소비를 바꾸지 않는다.
- 보유 수량이 0인 카드는 중간 회색 배경·점선 테두리·저채도 아이콘·`미보유` 문구로 비활성 상태처럼 구분한다. 조합법과 상세를 미리 볼 수 있도록 선택 동작은 유지한다. 모바일은 도감 상세처럼 슬롯·카탈로그 목록과 선택 상세를 위아래 독립 스크롤 영역으로 나눈다. 상세 헤더와 닫기 버튼, 하단 조합·장착 동작은 내용 스크롤 밖에 고정하며, 새 장비를 선택하면 상세 세로 위치를 처음으로 되돌리고 가로로 넘치는 조합 트리의 결과 노드를 중앙에 맞춘다. 닫기는 선택만 해제하고 장비 탭을 유지한다.
