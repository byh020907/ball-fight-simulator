...
> 상태: 조합 장비 도메인과 완성 장비 패시브 12종의 전투 로직 체크포인트 완료. 공통 VFX·디버그 표시는 후속 단계가 소유합니다.
...
따라서 완성 장비 12종은 모두 실제 전투 사건에 연결되어 있고, 후속 공통 VFX 체크포인트는 이 발동·위치·강도 정보를
사용할 수 있는 작은 장비 피드백 사건 경계만 추가하면 된다.
