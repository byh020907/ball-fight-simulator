...
- 카드에는 Canvas `x-equipment-icon-tag`, 정식명, 보유 수량, 원시 스탯을 보이고, 선택한 카드의 상세 영역에서 결과에서 재료로 내려가는 아이콘 조합 트리, 재료 `보유/필요`, 조합 비용과 결과 스탯을 확인한다. 조합 헤더의 `상위 조합`은 선택 장비를 직접 재료로 쓰는 장비만 템플릿 선언 순서로 보여 주고, 아이콘을 누르면 기존 선택 흐름으로 그 상세를 연다. 상위가 없으면 `최종 장비`를 표시한다. 역방향 레시피 관계와 카드 데이터는 `equipmentPresentation.js`가 소유하고, 트리는 정적 레시피 조회와 보유 수량 조회만 받는 읽기 전용 presentation으로 실제 조합 판단·저장·재료 소비를 바꾸지 않는다.
